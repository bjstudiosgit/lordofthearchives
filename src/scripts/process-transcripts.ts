...
Object.entries(newReports).forEach(([ep, report]) => {
  const barCount = Array.isArray(report.notableBars) ? report.notableBars.length : 0;
  if (barCount < TARGET_NOTABLE_BARS) {
    console.warn(
      `Report ${ep} currently has ${barCount} notable bars; target is ${TARGET_NOTABLE_BARS}.`,
    );
  }
});
