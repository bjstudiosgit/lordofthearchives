...
export default function BattleDetailClient({ slug }: { slug: string }) {
  ...
  const schemaData = battle.videoUrl ? {
    ...
    "description": `${mc1?.name ?? battle.mc1} faces ${mc2?.name ?? battle.mc2} in this PenGame battle.`,
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            >
              <h1 className="text-5xl md:text-7xl font-display italic uppercase leading-tight mb-8">
                <Link href={`/mc/${mc1?.slug}`} aria-label={`View ${mc1?.name}'s profile`} className="hover:text-brand transition-colors">{mc1?.name}</Link> <span className="text-brand">VS</span> <Link href={`/mc/${mc2?.slug}`} aria-label={`View ${mc2?.name}'s profile`} className="hover:text-brand transition-colors">{mc2?.name}</Link>
              </h1>
              ...
              <div className="grid grid-cols-2 gap-8 relative">
                ...
                <div className={`relative overflow-hidden rounded-2xl border p-8 text-center ${battle.winner === mc1?.id ? 'border-brand bg-brand/5 ring-2 ring-brand ring-offset-4 ring-offset-zinc-950' : 'border-white/5 bg-zinc-900/30'}`}>
                  ...
                  <div className={`relative z-10 ${battle.winner === mc1?.id ? 'mt-8' : ''}`}>
                    <Link href={`/mc/${mc1?.slug}`} aria-label={`View ${mc1?.name}'s profile`} className="text-3xl font-display italic uppercase hover:text-brand transition-colors">{mc1?.name}</Link>
                  </div>
                </div>
                ...
                <div className={`relative overflow-hidden rounded-2xl border p-8 text-center ${battle.winner === mc2?.id ? 'border-brand bg-brand/5 ring-2 ring-brand ring-offset-4 ring-offset-zinc-950' : 'border-white/5 bg-zinc-900/30'}`}>
                  ...
                  <div className={`relative z-10 ${battle.winner === mc2?.id ? 'mt-8' : ''}`}>
                    <Link href={`/mc/${mc2?.slug}`} aria-label={`View ${mc2?.name}'s profile`} className="text-3xl font-display italic uppercase hover:text-brand transition-colors">{mc2?.name}</Link>
                  </div>
                </div>
              </div>
